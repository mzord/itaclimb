defmodule ItaclimbWeb.UserAuthTest do
  use ItaclimbWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Itaclimb.Accounts
  alias Itaclimb.Accounts.Scope
  alias ItaclimbWeb.UserAuth

  import Itaclimb.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ItaclimbWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ItaclimbWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ItaclimbWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
